...
pubs_output_path = "./processed_ds/Aminer-WhoIsWho (na-v3)/processed/pubs_enriched.json"
auths_output_path = "./processed_ds/Aminer-WhoIsWho (na-v3)/processed/authors_extracted.json"


# JSON publications -> publications RDD: pub_id, title, list of authors
f = open(pubs_path)
pubs = json.load(f)
pubs_list = []
for pub in pubs:
...
# JSON authors -> authors RDD: pub_id, list of authors
f = open(auths_path)
keys = json.load(f)
author_info = []
for key in keys:  # clustering keys
...
# publications RDD: pub_id, title, list of authors
publications_rdd = pubs_rdd.leftOuterJoin(author_info_rdd).map(add).map(lambda x: x[1][0]).map(injectID)
# authors RDD: aut_id, info extracted from pub, ground_truth_id
